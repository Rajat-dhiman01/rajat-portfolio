  ...
  return (
    <div className={cn("text-base sm:text-xl md:text-3xl lg:text-5xl font-bold text-center", className)}>
        ...
        {wordsArray.map((word, idx) => (
          <div key={`word-${idx}`} className="inline-block">
            {word.text.map((char, index) => (
              <motion.span
                key={`char-${index}`}
                className={cn("opacity-0 hidden dark:text-white text-black", word.className)}
              >
                {char}
              </motion.span>
            ))}
      ...
      <motion.span
        ...
        className={cn("inline-block rounded-sm w-[4px] h-4 md:h-6 lg:h-10 bg-white", cursorClassName)}
      />
    </div>
  );
}
...
export function TypewriterEffectSmooth({ words, className, cursorClassName }) {
  ...
  return (
    <div className={cn("flex space-x-1 my-6", className)}>
      ...
      >
        <div className="text-xs sm:text-base md:text-xl xl:text-5xl font-bold whitespace-nowrap">
          {wordsArray.map((word, idx) => (
            <div key={`word-${idx}`} className="inline-block">
              {word.text.map((char, index) => (
      ...
      <motion.span
        ...
        animate={{ opacity: 1 }}
        transition={{ duration: 0.8, repeat: Infinity, repeatType: "reverse" }}
        className={cn("block rounded-sm w-[4px] h-4 sm:h-6 xl:h-12 bg-white", cursorClassName)}
      />
    </div>
  );
}
